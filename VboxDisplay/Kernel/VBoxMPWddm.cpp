...
NTSTATUS vboxWddmGhDisplaySetMode(PVBOXMP_DEVEXT pDevExt, const VBOXWDDM_ALLOC_DATA *pAllocData)
{
    ...
    USHORT width  = (USHORT)pAllocData->SurfDesc.width;
    USHORT height = (USHORT)pAllocData->SurfDesc.height;
    USHORT bpp    = (USHORT)pAllocData->SurfDesc.bpp;
...
//            WARN(("Do we need to do anything here?"));
    ...
    pHdr->u2.complexCmdEl.u16CbCmdHost = (uint16_t)cbPrivateData;
    pHdr->u2.complexCmdEl.u16CbCmdGuest = (uint16_t)cbBuffer;
...
//    LOGF(("ENTER, hContext(0x%x)", hContext));
    ...
    //            Assert(pRender->PatchLocationListOutSize == pRender->AllocationListSize);
        ...
        pCmd->u2.complexCmdEl.u16CbCmdHost = (uint16_t)cbPrivateData;
        pCmd->u2.complexCmdEl.u16CbCmdGuest = (uint16_t)cbBuffer;
    }
...
//    LOGF(("ENTER, hContext(0x%x)", hContext));
    ...
    else if (pPresent->Flags.Flip)
    {
        ...
        pHdr->u.u8PrimaryID = (uint8_t)pSrcAlloc->AllocData.SurfDesc.VidPnSourceId;
    ...
    )
{
    ...
    LOGREL(("VBox WDDM Driver for Windows %s version %d.%d.%dr%d, %d bit; Built %s %s",
            ...
            VBOX_VERSION_MAJOR, VBOX_VERSION_MINOR, VBOX_VERSION_BUILD, 1,
